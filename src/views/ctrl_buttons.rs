...
use cursive::{
    traits::{Nameable},
    views::{Button, NamedView, OnEventView, ResizedView, ListView}, view::Resizable, Cursive,
};

use crate::{actions::execute, herr, data::{BUTTONS, INSTALLER_CFG_OUT}, state::GlobalState};
...
pub fn buttons(final_state: bool) -> CmdLine {
    ...
    if final_state {
        l.add_child("", Button::new_raw("finish", |c| herr!(c, write_config_and_quit)));
    } else {
        l.add_child("", Button::new_raw("next", |c| herr!(c, execute, Move::Next)));
    }
